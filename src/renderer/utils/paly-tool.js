...
const playTool = {
        ...
        return new Promise((resolve, reject) => {
            this.getSite(key).then(res => {
                ...
                axios
                    .get(url)
                    .then(async res1 => {
                        const data = `${res1.data.match(/<list [^>]*>/)[0]}</list>`; // 某些源站不含页码时获取到的数据parser无法解析
                        const json = parser.parse(data, this.xmlConfig);
                        const jsondata = json.rss === undefined ? json : json.rss;
                        const pg = {
                            page: jsondata.list._page,
                    ...
                    axios
                        ...
                        .then(res1 => {
                            ...
                            if (json && jsondata && jsondata.list) {
                                ...
                                if (Object.prototype.toString.call(videoList) === '[object Object]')
                    ...
                    axios
                        ...
                        .then(res1 => {
                            ...
                            fullList.forEach(item => {
                                if (
                                    item.list.every(
                                        e => e.includes('$') && /^\s*\d+\s*$/.test(e.split('$')[0])
                                    )
                                )
                                    item.list.sort((a, b) => {
            ...
            this.getSite(key).then(res => {
                ...
                if (site.download) {
                            ...
                            if (type === '[object Array]') {
                                for (const i of dd) {
                                    downloadUrls = i._t
                                        .replace(/\$+/g, '$')
                                        .split('#')
                                        .map(e => encodeURI(e.includes('$') ? e.split('$')[1] : e))
                                        .join('\n');
                                }
                            } else {
                                downloadUrls = dd._t
                    ...
                    playTool
                        ...
                        .then(res1 => {
                            const dl =
                                res1.fullList.find(e => e.flag === videoFlag) || res.fullList[0];
    ...
    checkChannel(url) {
        return new Promise((resolve, reject) => {
            ...
            const extRE =
                url.match(supportFormats) || new URL.URL(url).pathname.match(supportFormats);
            if (extRE[1] === 'flv') {
                const MAX_CONTENT_LENGTH = 2000; // axios配置maxContentLength不生效，先用request凑合
                let receivedLength = 0;
                let options = { uri: url, gzip: true, timeout: 10000 };
                if (proxyURL) {
                    if (proxyURL.startsWith('http')) options = { proxy: proxyURL, ...options };
                    if (proxyURL.startsWith('socks5'))
                        options = { agent: new SocksProxyAgent(proxyURL), ...options };
                }
                const req = request
                    .get(options)
                    .on('data', str => {
    ...
    doubanLink(name, year) {
        return new Promise((resolve, reject) => {
            ...
            axios
                .get(doubanSearchLink)
                .then(res => {
                    const $ = cheerio.load(res.data);
                    // 查询所有搜索结果, 看名字和年代是否相符
                    let link = '';
                    $('div.result').each(() => {
                        const linkInDouban = $(this)
                            .find('div>div>h3>a')
                            .first();
                        const nameInDouban = linkInDouban.text().replace(/\s/g, '');
                        const subjectCast = $(this)
                            .find('span.subject-cast')
                            .text();
                        if (
                            nameToSearch === nameInDouban &&
                            subjectCast &&
    ...
    doubanRate(name, year) {
        return new Promise((resolve, reject) => {
            const nameToSearch = name.replace(/\s/g, '');
            this.doubanLink(nameToSearch, year)
                .then(link => {
    ...
    //                     webSession.setProxy({ proxyRules: proxyURL })
    //                     http.globalAgent = https.globalAgent = new ElectronProxyAgent(webSession)
    //                 }
    //             } else {
    //                 proxyURL = ''
    //                 webSession.setProxy({ proxyRules: 'direct://' })
    //                 http.globalAgent = https.globalAgent = new ElectronProxyAgent(webSession)
    //             }
    //             // 不要删了，留着测试用
