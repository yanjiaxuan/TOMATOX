    ...
    search (key, wd) {
        return new Promise((resolve, reject) => {
            this.getSite(key).then(res => {
                ...
                axios.get(url, { timeout: 3000 }).then(res1 => {
                    const {data} = res1
                    const json = parser.parse(data, this.xmlConfig)
                    const jsondata = json?.rss === undefined ? json : json.rss
        ...
        return new Promise((resolve, reject) => {
            ...
            this.getSite(key).then(res => {
                ...
                if (site.download) {
                    ...
                    playTool.detail(key, id).then(res1 => {
                        const dl = res1.fullList.find(e => e.flag === videoFlag) || res.fullList[0]
        ...
        return new Promise((resolve, reject) => {
            ...
            if (extRE[1] === 'flv') {
                ...
                if (proxyURL) {
                    ...
                    if (proxyURL.startsWith('socks5')) options = {agent: new SocksProxyAgent(proxyURL), ...options}
                }
